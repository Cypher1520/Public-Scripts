...
<#------------------
Invoke-Command -ComputerName 'Cgyprddc01' -ScriptBlock {
$filter = "<QueryList>" + `
    "<Query Id=`"0`" Path=`"Security`">" + `
        "<Select Path=`"Security`">" + `
            "*[System[(EventID=4634 or EventID=4624) and " + `
            "TimeCreated[@SystemTime&gt;='2019-01-01T06:00:00Z' and @SystemTime&gt;'2019-01-03T06:00:00Z']]] and " + `
...
Get-WinEvent -FilterXML $filter -MaxEvents 5 | 
%{ [xml]$xml = $_.ToXml()
