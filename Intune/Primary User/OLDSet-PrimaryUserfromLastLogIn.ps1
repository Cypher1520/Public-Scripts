...
Source:
https://github.com/svdbusse/IntuneScripts/blob/master/PrimaryUser/Set-PrimaryUserfromLastLogIn.ps1

Logging for changes located in last section for each write-host line, creates log in "C:\Temp\Output"
#>
...
if (-not(Test-Path "C:\Temp\Output")) {
    New-Item "C:\Temp\Output" -ItemType Directory
}
...
Start-Transcript -Path C:\Temp\Output\Transcript.txt
Write-Host "Getting all devices for tenant"
$Devices = Get-Win10IntuneManagedDevice
Foreach ($Device in $Devices) { 

    Write-Host "Device name:" $device."deviceName" -ForegroundColor Cyan
    #Write-Output "Device name:" $device."deviceName" >> C:\Temp\Output\CurrentPrimaryUser.txt
    ...
    if ($IntuneDevicePrimaryUser -eq $null) {
        ...
        #Write-Output "No Intune Primary User Id set for Intune Managed Device" $Device."deviceName" "----" >> C:\Temp\Output\NoPrimaryUser.txt
        ...
        Write-Output "Intune Device Primary User:" $PrimaryAADUser.displayName "----" >> C:\Temp\Output\CurrentPrimaryUser.txt
    ...
    if ($IntuneDevicePrimaryUser -notmatch $User.id) {
        ...
        $SetIntuneDevicePrimaryUser = Set-IntuneDevicePrimaryUser -IntuneDeviceId $Device.id -userId $User.id -ErrorAction SilentlyContinue -ErrorVariable PrimaryUserError
        ...
        if ($SetIntuneDevicePrimaryUser -eq "") {
            ...
            #Write-Output "User"$User.displayName"set as Primary User for device '$($Device.deviceName)'..." "----" >> C:\Temp\Output\NewPrimaryUser.txt
        }
        ...
        #Write-Output "'$($Device.deviceName)'..." >> C:\Temp\Output\Errors.txt
        #Write-Output "$PrimaryUserError" "----" >> C:\Temp\Output\Errors.txt
        $PrimaryUserError = ""
    }   
