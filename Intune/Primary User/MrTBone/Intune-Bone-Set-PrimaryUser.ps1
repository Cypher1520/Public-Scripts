...
.DESCRIPTION
    Source: https://www.tbone.se/2025/03/31/set-intune-primary-user-with-azure-automation/
...
param(
    ...
    [string[]]$OperatingSystems     = @('All'),
    
    [Parameter(Mandatory = $false,          HelpMessage = "Filter to only include devicenames that starts with ('Example1', 'Desktop'). Default is blank")]
    [string[]]$IncludedNames        = @(),
    
    [Parameter(Mandatory = $false,          HelpMessage = "Filter to exclude devicenames that starts with ('RIS', 'Desktop'). Default is blank")]
    [string[]]$ExcludedNames        = @(),

    [Parameter(Mandatory = $false,          HelpMessage = "Filter to exclude enrollment accounts ('example@r-is.tech','chris@r-is.tech'). Default is blank")]
    [string[]]$EnrollmentAccounts   = @(""),
